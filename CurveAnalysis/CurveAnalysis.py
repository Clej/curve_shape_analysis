...
def calculate_velocity(DX1):
    return [norm(dxi) for dxi in DX1.T]


def calculate_arc_length(DX1, t):
    n = len(t)
    t0 = t[0]
    v = calculate_velocity(DX1)
    v0 = v[0]
    return [simps(y=v[:i+1], x=t[:i+1]) for i in range(n)]


def calculate_curvature(DX1, DX2):
    def norm2(x):
...
class CurveAnalysis:
    ...
    def __init__(self, grid: skfda.FDataGrid):
        ...
        self.nSeries = self.init_grid.data_matrix.shape[0]
        self.nObs = self.init_grid.data_matrix.shape[1]
        self.nVar = self.init_grid.data_matrix.shape[2]
        self.cordinate_grids = list(self.init_grid.coordinates)
        self.smoothed = False
        self.scaled = False
    ...
    def smooth_grids(self,
        ...
        smoothed_grids = []
        for data_grid in tqdm(self.grids):
        ...
        self.grids = smoothed_grids
        self.smoothed = True
        self.grids_dx1 = [grid.derivative(1) for grid in self.grids]
        self.grids_dx2 = [grid.derivative(2) for grid in self.grids]
        return None
    ...
    def compute_velocity(self):
        if not self.smoothed:
            _ = self.smooth_grids()
        dx1_mat = np.empty([self.nVar, self.nObs])
        result_matrix = np.empty([self.nSeries, self.nObs])
        for i in range(self.nSeries):
            for j in range(self.nVar):
                dx1_mat[j, :] = self.grids_dx1[j].data_matrix[i, :, 0]
            result_matrix[i, :] = calculate_velocity(DX1=dx1_mat)
    ...
    def compute_arc_length(self):
        ...
        for i in range(self.nSeries):
            for j in range(self.nVar):
                dx1_mat[j, :] = self.grids_dx1[j].data_matrix[i, :, 0]
            result_matrix[i, :] = calculate_arc_length(
    ...
    def compute_curvature(self):
        if not self.smoothed:
            _ = self.smooth_grids()
        dx1_mat = np.empty([self.nVar, self.nObs])
        dx2_mat = np.empty([self.nVar, self.nObs])
        result_matrix = np.empty([self.nSeries, self.nObs])
        for i in range(self.nSeries):
            for j in range(self.nVar):
                dx1_mat[j, :] = self.grids_dx1[j].data_matrix[i, :, 0]
                dx2_mat[j, :] = self.grids_dx2[j].data_matrix[i, :, 0]
            result_matrix[i, :] = calculate_curvature(DX1=dx1_mat, DX2=dx2_mat)
        return skfda.FDataGrid(data_matrix=result_matrix,
